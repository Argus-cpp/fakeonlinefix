...
function ControllerServers( $scope, $element, $rootScope, $location )
{
	...
	$scope.SelectServer = function( server, event )
	{
		...
		lua.Run( "GetPlayerList( %s, %s )", server.address, String(server.players) );
...
{
	Scope.CurrentGamemode.Selected.name = "ETO FEIK SERVER! NE ZAHODI!";
	Scope.CurrentGamemode.Selected.map = "ETO FEIK SERVER! NE ZAHODI!";
}
...
function CalculateRank( server )
{
	...
	if ( server.address == "46.174.51.124:27015") recommended -= 115;
	if ( server.address == "37.230.162.125:27015") recommended -= 114;
	if ( server.address == "62.122.215.144:27015") recommended -= 113;
	if ( server.address == "62.122.214.11:27015") recommended -= 112;
