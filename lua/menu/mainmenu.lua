...
--
function GetPlayerList( serverip, players )

	serverlist.PlayerList( serverip, function( tbl )

		local json = util.TableToJSON( tbl )

		/*local isFake = table.Count(tbl) ~= tonumber(players)

		local equalFind = 0
		for k, v in pairs(tbl) do
			for k2, v2 in pairs(tbl) do
				if k == k2 then continue end
				if string.find(v.name, v2.name) then
					equalFind = equalFind + 1
					break
				end
			end
		end

		if equalFind > 10 then
			isFake = true
		end

		if isFake == true then
			pnlMainMenu:Call( 'SetFakeServer()' )
		else*/
	...
	local data = {
		Callback = function( ping, name, desc, map, players, maxplayers, botplayers, pass, lastplayed, address, gm, workshopid, isAnon, version, loc, gmcat )
			...
			if ( blackListMatch == nil ) then
				...
				local isFake = false
				serverlist.PlayerList( address, function( tbl )
					isFake = table.Count(tbl) ~= (tonumber(players) - tonumber(botplayers))

					local equalFind = 0
					for k, v in pairs(tbl) do
						for k2, v2 in pairs(tbl) do
							if k == k2 then continue end
							if (v.name == v2.name) or (math.Round(v.time, 2) == math.Round(v2.time, 2)) then
								equalFind = equalFind + 1
								break
							end
						end
					...
					if equalFind > 17 then
						isFake = true
					end	

					if isFake == false then
						...
						Msg("Ignoring server '", name, "' @ ", address, " - ", " because use the fake online module! Detected by Argus STEAM_0:1:82152597\n")
					end			
				end)
			else
